  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-5 py-4">
        ...
        <Link
          href="/expenses"
          className="inline-flex items-center gap-1 text-xs font-medium text-muted-foreground transition-colors hover:text-ink-soft dark:hover:text-[#a3a5a0]"
        >
      ...
      {!hasItems ? (
        <div className="px-5 py-8 text-center">
          <motion.div
            ...
            className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-secondary dark:bg-white/5"
          >
        ...
        >
          {recentItems.map((item) => (
            <motion.div key={item.id} variants={staggerItem}>
              <Link
                href={item.href}
                className="flex items-center gap-4 border-t border-border px-5 py-3 transition-colors last:rounded-b-xl hover:bg-background dark:border-[rgba(255,255,255,0.04)] dark:hover:bg-secondary"
              >
                <div
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg"
                  style={{
                    backgroundColor:
                      CATEGORY_COLORS[item.category] ?? "#eef1f7",
                  }}
                  aria-hidden
                />
                ...
                <div className="shrink-0 text-right">
                  <p className="stat-number text-[13px] font-medium text-foreground dark:text-foreground">
                  ...
                  <div className="mt-0.5 flex items-center justify-end gap-1.5">
                    {item.type === "full" ? (
                      ...
                      <Badge className="h-5 gap-1 rounded-full border-0 bg-secondary px-2 text-[10px] font-medium text-muted-foreground dark:bg-white/5">
